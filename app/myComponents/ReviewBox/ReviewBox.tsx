...
const review: React.FC<Reveiw> = ({ rating, username, feedback,date}) => {
  return (
    <div className="flex flex-col gap-2 border-[1px] rounded-[1.25rem] p-6 ">
        <div className="flex gap-1">
      {Array.from({ length: rating }, (_, i) => (
        <span key={i}>{Star}</span>
      ))}
        </div>
      <span className="font-satoshi font-bold text-left text-base flex items-center gap-2">{username}   {Verification}</span>
      <p className="font-satoshi font-normal text-left text-[#00000099] text-sm">`{`"${feedback}"}`}</p>
      {date && ( 
        <div className="text-sm  font-satoshi text-[#00000099] mt-3">
          {`Posted On: ${date}`}
        </div>
      )}
    </div>
